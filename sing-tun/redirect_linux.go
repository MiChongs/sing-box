...
// checkIP6TablesNATSupport checks whether ip6tables NAT functionality is actually available
// by attempting to list the nat table. If the ip6table_nat kernel module is not loaded, this will fail.
func (r *autoRedirect) checkIP6TablesNATSupport(ip6tablesPath string) bool {
	var cmd *exec.Cmd
	if r.androidSu {
		cmd = exec.Command(r.suPath, "-c", ip6tablesPath+" -w 5 -t nat -L -n")
	} else {
		cmd = exec.Command(ip6tablesPath, "-w", "5", "-t", "nat", "-L", "-n")
	}
	return cmd.Run() == nil
}
...
func (r *autoRedirect) Start() error {
	...
	if runtime.GOOS == "android" {
		...
		// Try to enable IPv6 on Android if ip6tables is available and has NAT support
		if len(r.tunOptions.Inet6Address) > 0 {
			ip6tablesPath := "/system/bin/ip6tables"
			if _, statErr := os.Stat(ip6tablesPath); statErr == nil {
				if r.checkIP6TablesNATSupport(ip6tablesPath) {
					r.enableIPv6 = true
					r.ip6tablesPath = ip6tablesPath
					r.logger.Info("IPv6 NAT enabled on Android via ip6tables")
				} else {
					r.logger.Warn("ip6tables found but IPv6 NAT not supported (ip6table_nat module may be missing), falling back to IPv4 only")
				}
			} else {
				r.logger.Info("ip6tables not found on Android, IPv6 NAT disabled")
			}
		}
	} else {
		if r.useNFTables {
			...
			if !r.useNFTables {
				...
				if err != nil {
					if !r.enableIPv4 {
						...
						r.enableIPv6 = false
						r.logger.Warn("ip6tables not found, falling back to IPv4 only: ", err)
					}
				} else if !r.checkIP6TablesNATSupport(r.ip6tablesPath) {
					// ip6tables binary exists but ip6table_nat kernel module is not available
					if !r.enableIPv4 {
						return E.New("ip6tables found but IPv6 NAT not supported (ip6table_nat module may be missing)")
					}
					r.enableIPv6 = false
					r.ip6tablesPath = ""
					r.logger.Warn("ip6tables found but IPv6 NAT not supported (ip6table_nat module may be missing), falling back to IPv4 only")
	...
	if r.customRedirectPort == 0 {
		...
		if runtime.GOOS == "android" && !r.enableIPv6 {
