...
type System struct {
	...
	inet4LoopbackAddressSet map[netip.Addr]struct{}
	inet6LoopbackAddressSet map[netip.Addr]struct{}
...
func NewSystem(options StackOptions) (Stack, error) {
	...
	stack.inet4LoopbackAddressSet = make(map[netip.Addr]struct{}, len(stack.inet4LoopbackAddress))
	for _, addr := range stack.inet4LoopbackAddress {
		stack.inet4LoopbackAddressSet[addr] = struct{}{}
	}
	stack.inet6LoopbackAddressSet = make(map[netip.Addr]struct{}, len(stack.inet6LoopbackAddress))
	for _, addr := range stack.inet6LoopbackAddress {
		stack.inet6LoopbackAddressSet[addr] = struct{}{}
	}
...
func (s *System) processIPv4TCP(ipHdr header.IPv4, tcpHdr header.TCP) (bool, error) {
	...
	} else if _, isLoopback := s.inet4LoopbackAddressSet[destination.Addr()]; isLoopback {
...
func (s *System) processIPv6TCP(ipHdr header.IPv6, tcpHdr header.TCP) (bool, error) {
	...
	if !destination.Addr().IsGlobalUnicast() {
		...
		ipHdr.SetDestinationAddr(session.Source.Addr())
		tcpHdr.SetDestinationPort(session.Source.Port())
	} else if _, isLoopback := s.inet6LoopbackAddressSet[destination.Addr()]; isLoopback {
		ipHdr.SetDestinationAddr(ipHdr.SourceAddr())
		ipHdr.SetSourceAddr(destination.Addr())
	} else {
		natPort, err := s.tcpNat.Lookup(source, destination, s.handler)
