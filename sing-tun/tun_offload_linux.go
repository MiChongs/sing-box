...
// is found. Single map lookup — avoids the redundant second lookup in insert().
func (t *tcpGROTable) lookupOrInsert(pkt []byte, srcAddrOffset, dstAddrOffset, tcphOffset, tcphLen, bufsIndex int) ([]tcpGROItem, bool) {
	...
	// Insert directly with the key we already have — no second lookup
	item := tcpGROItem{
		key:       key,
...
func (t *tcpGROTable) newItems() []tcpGROItem {
	if len(t.itemsPool) == 0 {
		return make([]tcpGROItem, 0, idealBatchSize)
	}
...
func (u *udpGROTable) newItems() []udpGROItem {
	if len(u.itemsPool) == 0 {
		return make([]udpGROItem, 0, idealBatchSize)
	}
