...
func newGVisorStack(ep stack.LinkEndpoint, opts stack.NICOptions, allowRawEndpoint bool, isLocalStack bool) (*stack.Stack, error) {
	...
	// Enable delayed ACK for better throughput (coalesces multiple ACKs into one)
	dOpt := tcpip.TCPDelayEnabled(true)
	ipStack.SetTransportProtocolOption(tcp.ProtocolNumber, &dOpt)
